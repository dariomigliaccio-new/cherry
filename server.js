...
function renderLayout({ title, content, activePath = "/", home = false }) {
  ...
  <html lang="en">
    <head>
      ...
      <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Playfair+Display:ital,wght@0,700;0,800;1,400&display=swap" rel="stylesheet">
...
app.get("/manager", requireAdmin, (_req, res) => {
  ...
  sections.push(`<section class="section-availability"><h2>Unit Availability</h2><div class="avail-admin-stats"><div class="avail-admin-stat avail-admin-stat--total"><strong>${uTotal}</strong><span>Total Units</span></div><div class="avail-admin-stat avail-admin-stat--open"><strong>${uAvail}</strong><span>Available Now</span></div><div class="avail-admin-stat avail-admin-stat--taken"><strong>${uTaken}</strong><span>Units Leased</span></div></div>${field("Total Units", "pages./sustainability.details.unitsValue", data)}<p class="avail-admin-note">Enter how many units are still available per bedroom type. Units Leased and Available Now update automatically.</p><div class="br-cols"><div class="br-col"><h4 class="br-col-title">1 Bedroom <span class="br-col-fixed">${BR1_TOTAL} total</span></h4>${field("Available", "pages./sustainability.details.available1BR", data, "number")}</div><div class="br-col"><h4 class="br-col-title">2 Bedrooms <span class="br-col-fixed">${BR2_TOTAL} total</span></h4>${field("Available", "pages./sustainability.details.available2BR", data, "number")}</div><div class="br-col"><h4 class="br-col-title">3 Bedrooms <span class="br-col-fixed">${BR3_TOTAL} total</span></h4>${field("Available", "pages./sustainability.details.available3BR", data, "number")}</div></div></section>`);
